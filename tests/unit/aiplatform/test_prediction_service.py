...
class TestPredictor:
    ...
    def test_constructor_with_conflicting_project(self):
        """Passing a full resource name with `_TEST_PROJECT` and providing `_TEST_PROJECT_2` as project"""

        with pytest.raises(ValueError) as err:
            prediction_service.Predictor(
                endpoint_name=_TEST_ENDPOINT_NAME, project=_TEST_PROJECT_2
            )

        assert err.match(
            regexp=r"is different from the resource project"
        )
    ...
    def test_constructor_with_conflicting_location(self):
        ...
        assert err.match(
            regexp=r"is different from the resource location"
        )
