...
class TestPipelineJobSchedule(e2e_base.TestEndToEnd):
    ...
    def test_create_get_pause_resume_update_list(self, shared_state):
        ...
        assert pipeline_job_schedule.cron == cron_expression

        # Updating the pipeline job schedule.
        new_cron_expression = "* * * * *"
        pipeline_job_schedule.update(cron=new_cron_expression)
        assert pipeline_job_schedule.cron == new_cron_expression
